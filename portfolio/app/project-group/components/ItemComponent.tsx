...
const ItemComponent: React.FC<{ item: Item }> = ({ item }) => {
  ...
  // useEffect를 사용하여 모달이 열릴 때 body의 스크롤을 제어
  useEffect(() => {
    if (modalImage) {
      // 모달이 열릴 때 body 스크롤을 숨김
  ...
  const openNotion = () => {
    const notionLinks: { [key: string]: string } = {
      'ERP System': 'https://kongukjae.notion.site/ERP-abc3f61cb1ce4a52859ab0f5c4aedec4?pvs=4',
      '랜덤 포켓몬': 'https://nifty-mountain-1e6.notion.site/24-05-11-24-05-13-5ec60ad525b14e139fff5f5ba9da24da?pvs=4',
      '스트레스 블로그': 'https://nifty-mountain-1e6.notion.site/blog-24-05-24-24-07-07-192fa48e9c8c4b639f3b85e51022c73a?pvs=4',
      '영양제 조절 사이트': 'https://nifty-mountain-1e6.notion.site/f11ef4686e184f92a7c75d82cd3b35e6?pvs=4',
      '여행 공유 사이트': 'https://nifty-mountain-1e6.notion.site/Plan-T-decd39d6377342d0abfd94398062880e?pvs=4',
    };

    const link = notionLinks[item.projectName];
    if (link) {
  ...
  return (
    ...
    >
      ...
      <div className="w-full flex flex-col items-center mb-4">
        {item.image && <img src={item.image} alt={item.name} className="max-w-full h-auto object-contain mb-4" />}
      </div>
      ...
      {modalImage && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white rounded-lg relative w-11/12 md:w-4/5 md:h-5/6 h-4/6 flex flex-col">
            <button className="absolute top-4 right-4 text-gray-700 hover:text-gray-500 text-3xl font-bold" onClick={closeModal} aria-label="Close Modal">
              &times;
            </button>

            <div className="flex-1 flex flex-col items-center justify-center overflow-hidden p-4 md:p-16">
              <Image
                src={modalImage}
            ...
            {/* 썸네일 이미지들 */}
            {images.length > 0 && (
              <div className="flex overflow-x-auto mt-4 py-2 border-t border-gray-200 px-4">
                {images.map((src, idx) => (
                  <div
                    key={idx}
                    className={`w-24 h-24 cursor-pointer relative flex-shrink-0 border-2 ${
                      currentIndex === idx ? 'border-gray-400' : 'border-transparent'
                    } transition-transform transform hover:scale-105 mr-2`}
                    onClick={() => {
                      setModalImage(src);
                      setCurrentIndex(idx);
                    }}
          ...
          <button
            ...
            className="hidden md:block absolute top-1/2 left-4 transform -translate-y-1/2 text-gray-400 text-5xl font-bold p-2 hover:text-gray-600"
          ...
          <button
            ...
            className="hidden md:block absolute top-1/2 right-4 transform -translate-y-1/2 text-gray-400 text-5xl font-bold p-2 hover:text-gray-600"

      ...
      {modalImage && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded-lg relative max-w-5xl w-full mx-4">
            ...
            <Image src={modalImage} alt="ERP Details" width={1200} height={900} className="object-contain w-full h-auto" />
            ...
            {item.projectName === 'ERP System' && (
              <div className="flex overflow-x-auto mt-4 py-2 border-t border-gray-200">
                {images.map((src, idx) => (
                  ...
                  >
                    <Image src={src} alt={`ERP Thumbnail ${idx + 1}`} layout="fill" objectFit="cover" className="rounded shadow" />
                  </div>
                ))}
              </div>
            )}
